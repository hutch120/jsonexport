/**
 * !!! Modified for Field Data. !!!
 * Added option to return Horizontal Rows Array with headers in [0] and data in [1]
 */
 ...
 */
var options = {
    returnHorizontalRowsArray: true, // Boolean
 ...
 */
jsonexport = function (json, userOptions, callback) {
...
function generateCsv(json, callback) {
    ...
    if (_.isArray(json)) {
        ...
        //Merge all rows in a single output with the correct End of Line string
        var outputFile = fileRows.join(options.endOfLine || os.EOL || '\n');
        if (returnHorizontalRowsArray) {
            return callback(null, horizontalRows);
        } else {
            return callback(null, outputFile);
        }
    }
